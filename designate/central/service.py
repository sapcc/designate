...
class Service(service.RPCService):
    ...
    def _enforce_recordset_quota(self, context, zone):
        ...
        self.quota.limit_check(
            context, tenant_id, zone_recordsets=count + 1)
    ...
    def _generate_record_serial(self, zone, record, increment_serial):
        if not increment_serial:
            record.serial = zone.serial
        else:
            current_time = timeutils.utcnow_ts()
            if hasattr(record, 'serial') and record.serial:
                if current_time > record.serial:
                    record.serial = current_time
                else:
                    record.serial += 1
            else:
                record.serial = current_time
    ...
    def move_zone(self, context, zone):
        ...
        db_zone = self.storage.get_zone(context, zone.id)
        orig_pool_id = db_zone.pool_id
        changes = zone.obj_get_changes()
        target_pool_id = changes.get('pool_id')
    ...
    def create_zone_transfer_request(self, context, zone_transfer_request):
        ...
        for share in shared_zones:
            ...
            if (zone_transfer_request.target_tenant_id ==
