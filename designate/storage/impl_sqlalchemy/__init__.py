...
class SQLAlchemyStorage(sqlalchemy_base.SQLAlchemy, storage_base.Storage):
    ...
    def delete_zone(self, context, zone_id):
        ...
        if zone.shared:
            query = (
                tables.zones.update()
                .where(tables.zones.c.id == zone.id)
                .where(
                    tables.zones.c.id == tables.shared_zones.c.zone_id
                )
            )
        ...
        return self._delete(context, tables.zones, zone,
                            exceptions.ZoneNotFound, soft_delete_query=query)
    ...
    def update_zone_share(self, context, share):
        query = (
            tables.shared_zones.update().where(
                tables.shared_zones.c.id == share.id
            )
        )
    ...
    def find_recordsets(self, context, criterion=None, marker=None, limit=None,
        ...
        return self._find_recordsets(context, criterion, marker=marker,
            sort_dir=sort_dir, sort_key=sort_key, limit=limit,
            force_index=force_index,
            apply_tenant_criteria=apply_tenant_criteria)
