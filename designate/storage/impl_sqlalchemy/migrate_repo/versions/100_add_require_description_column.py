...
def upgrade(migrate_engine):
    LOG.info(_LI("Adding boolean column require_description to table 'zones'"))
    meta.bind = migrate_engine
    zones_table = Table('zones', meta, autoload=True)
    col = Column('require_description', Boolean(), default=False)
    col.create(zones_table)
    index = Index('require_description', zones_table.c.delayed_notify)
