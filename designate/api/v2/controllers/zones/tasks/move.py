...
class MoveController(rest.RestController):
        ...
        if 'pool_id' in body:
            if zone.pool_id == body['pool_id']:
                raise exceptions.BadRequest(
                    'Target pool should be different for zone move')

        zone = DesignateAdapter.parse('API_v2', body, zone)
        zone.validate()

        LOG.info("Triggered move for %(zone)s", {'zone': zone})
        zone = self.central_api.move_zone(context, zone)
