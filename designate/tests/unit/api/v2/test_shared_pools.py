...
class ApiV2SharedPoolsTest(v2.ApiV2TestCase):
    ...
    def test_share_response_links_contain_pool_not_zone(self):
        response = self._create_valid_shared_pool()
        links = response.json['links']
        self.assertIn('pool', links)
        self.assertNotIn('zone', links)

    def test_share_pool_duplicate_returns_409(self):
        self._create_valid_shared_pool()
        ...
        self.assertEqual('duplicate_shared_pool', response.json['type'])

    def test_delete_nonexistent_share_returns_404(self):
        fake_id = generate_uuid()
        self._assert_exception(
            'shared_pool_not_found', 404,
            self.client.delete,
            '{}/{}'.format(
                self.endpoint_url.format(self.pool.id), fake_id
            ),
            headers={'X-Test-Role': 'admin'}
        )

    def test_list_shares_for_different_pool_returns_empty(self):
        self._create_valid_shared_pool()
        other_pool = self.create_pool(
            fixture=1, domain_id='other-domain'
        )
        response = self.client.get(
            self.endpoint_url.format(other_pool.id),
            headers={'X-Test-Role': 'admin'}
        )
        self.assertEqual(0, len(response.json['shared_pools']))
